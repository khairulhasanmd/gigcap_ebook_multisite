        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">Tags</li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div class="card-header border-0 pt-6">
                            ...
                            <div class="card-toolbar">
                                <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base"> <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add_edit_modal">Add Tag</button> </div>
                            ...
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_customers_table">
                                <thead>
                                    <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                        ...
                                        <th class="min-w-125px">Tag Name</th>
                        ...
                        <form class="form" action="{{url('/').'/'.Request::path()}}" enctype="multipart/form-data" id="add_edit_modal_form" data-kt-redirect="{{url('/').'/'.Request::path()}}" method="POST">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <?php (isset($detail)) ? '<input type="hidden" name="concept_id" id="id" value="'.$concept_id.'' : '">'; ?>
                                    @csrf
                                    <input type="hidden" name="concept_id" value="{{$concept_id}}">

                                    <div class="modal-header" id="add_edit_modal_header">
                                        <h2 class="fw-bold" id="ModalTitle">{{ (isset($detail)) ? 'Edit' : 'Add' }} a Tag</h2>

                                        <div id="add_edit_modal_close" onclick="window.location.href='{{ url('/admin/tags') }}'" class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal"> <i class="ki-duotone ki-cross fs-1"> <span class="path1"></span> <span class="path2"></span> </i> </div>
                                    </div>
                                    <div class="modal-body py-10 px-lg-17">
                                        <div class="scroll-y me-n7 pe-7" id="add_edit_modal_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#add_edit_modal_header" data-kt-scroll-wrappers="#add_edit_modal_scroll" data-kt-scroll-offset="300px">
                                            <div class="fv-row mb-7">
                                                ...
                                                <input type="text" class="form-control form-control-solid @error('name') is-invalid @enderror" placeholder="Tag Name" onkeyup="document.getElementById('slug').value=slugify(this.value);" name="name" value="{{ (isset($detail)) ? $detail->name : '' }}" />
                                                @error('name')
                                            ...
                                            <div class="fv-row mb-7">
                                                <label class="required fs-6 fw-semibold mb-2">Select Category</label>
                                                <select class="form-control @error('type') is-invalid @enderror" name="category_id" id="type">
                                                    @foreach($categorys as $category)
                                                        <option value="{{ $category->id }}" {!! isset($detail) && $detail->category_id == $category->id ? 'selected' : '' !!}>{{ $category->name }}</option>
                                                    @endforeach
                                                </select>
                                    ...
                                    <div class="modal-footer flex-center">
                                        <button type="button" class="btn btn-light me-3" onclick="window.location.href='{{ url('/admin/tags') }}'" >Discard</button>
